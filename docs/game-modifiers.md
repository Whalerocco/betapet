...
- A displaced tile is temporarily restricted: for the remainder of the turn in which it was displaced, if the same player plays it again that turn, it may not end up standing in for another tile displaced this move — no replace-chaining within one move. The restriction is about that **outcome**, not only about the act of displacing (DEC-033): it is broken just as much by moving onto a square that is *already* standing in for a displaced tile as by displacing one directly, and both are refused. It may still be placed freely on an empty cell, and may still be swapped with one of the player's own not-yet-played tiles where that square stands in for nothing. Once at least one full turn has passed with the tile still in the player's rack, the restriction lifts and the tile becomes usable exactly like any other rack tile, including for another replace placement.
...
- A replace placement may only target a *committed* board tile. Dropping a tile onto one of the current player's own not-yet-committed tiles is not a replace placement at all: it is ordinary editing of an unplayed move, allowed in every game mode with or without this modifier, and it simply swaps the two — the tile that was on the square returns to the rack (DEC-017). None of this section's restrictions apply to it, since nothing has been played, nothing leaves the board, and no opponent tile is involved. The one carry-over is that the square keeps whatever displacement it already stood for: if an earlier placement this move displaced a committed tile there, the incoming tile becomes the one replacing it, and may therefore not show that displaced tile's own letter (DEC-015 again) — and, for the same reason, may not itself be a tile displaced this move, since it would then be standing in for a second displaced tile (DEC-033). Those two are the only points on which this bullet's "none of this section's restrictions apply" is qualified, and both follow from the carry-over rather than from the swap: a swap onto a square that stands in for nothing inherits nothing and is unrestricted.
...
- The all-tiles bonus is unaffected by the rule above: it is awarded for emptying the rack (section 25), so a rack-emptying move earns it even if every word it touched scored nothing. Replace mode does interact with that bonus in a different way, though: since a displaced tile joins the replacing player's own rack, a hand can hold more tiles than the configured rack size, and the bonus is earned by emptying whatever the hand holds rather than by placing exactly a rack's worth (`game-rules.md` section 25, DEC-018).
